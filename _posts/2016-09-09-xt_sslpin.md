...
[xt_sslpin] extracts certificates transported in the server's certificate message, generates finger prints and matches them against finger print lists. Then it starts matching the rules. Other that the [fredburger/xt_sslpin] the matching won't be delayed until the servers changeCipherSpec message was seen.
