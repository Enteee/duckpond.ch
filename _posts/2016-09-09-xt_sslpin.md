...
[xt_sslpin] works based on finger prints. A finger print is a [SHA1] hash of a x509 and DER encoded certificate. A collection of finger prints is called a finger print list. These lists are the key part when we specify an [iptables] rule:

> iptables -I \<chain\> .. -m sslpin [!] --fpl \<finger print list id\> ..

[xt_sslpin] extracts certificates transported in the server's certificate message, generates finger prints and matches them against finger print lists. Then it starts matching the rules. Other than in [fredburger/xt_sslpin] won't the matching be delayed until the servers changeCipherSpec message. This change allows for early stage abortion of malicious connections.
...
You can retrieve the finger print of the github.com certificate using the following command:
...
For managing finger print lists the kernel exposes a user space API under `/sys/kernel/xt_sslpin/`. Using this API should be straightforward.
...
Adding the just generated github.com finger print to list `0` is as simple as:

```shell
echo d79f076110b39293e349ac89845b0380c19e2f8b > /sys/kernel/xt_sslpin/0_add
```

The finger print list `0` can now be used in [iptables] as follows:
